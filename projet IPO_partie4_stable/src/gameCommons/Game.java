...
public class Game {
	...
	public void update() {
		...
		if (testLose() || this.endGameLose) {
			...
			if (frog.getDist(1) == -1)
				...
				graphic.endGameScreen("GAME OVER", frog.getDist(2), timer);
			return;
		}
		...
		if (testWin()) {
			...
			if (frog.getDist(1) == -1)
				...
				graphic.endGameScreen("YOU WIN", frog.getDist(2), timer);
			return;
		}
	}
	...
	public void setFrogDist(int new_dist) {
		frog.setDist(new_dist);
	}

	public Case getFrogNextPos() {
		return frog.getNextPosition();
	}
}
